...
class HomeRecommendContainerCell: UICollectionViewCell {
    ...
    static func height(viewModel: HomeRecommendViewModel) -> CGFloat {
        let top: CGFloat = 84 - 6 // 첫번째 cell에서 botoom까지 거리 - cell의 상단 여백
...
//    private var recommends: [Home.Recommend]?
    ...
    @IBAction func foldButtonDidTap(_ sender: Any) {
        ...
        self.delegate?.homeRecommedCountainerCellFoldChanged(self)
    }
...
extension HomeRecommendContainerCell: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        self.delegate?.homeRecommendContainerCell(self, didSelectedItemAt: indexPath.row)
    }
