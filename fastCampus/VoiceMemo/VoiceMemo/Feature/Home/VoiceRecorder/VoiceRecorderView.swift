...
struct VoiceRecorderView: View {
    ...
    var body: some View {
        ZStack {
            ...
            RecordBtnView(voicRecorderViewModel: voiceRecorderViewModel)
                .padding(.trailing, 20)
                .padding(.bottom, 50)
        }
        .alert(
        ...
        .onChange(
            ...
            perform: {recorderFile in
                homeViewModel.setVoiceRecorderCount(recorderFile.count)
            }
        )
    }
}
...
private struct RecordBtnView: View {
    @ObservedObject private var voicRecorderViewModel: VoiceRecorderViewModel
    @State private var isAnimation: Bool
    
    fileprivate init(
        voicRecorderViewModel: VoiceRecorderViewModel,
        isAnimation: Bool = false
    ) {
        self.voicRecorderViewModel = voicRecorderViewModel
    ...
    fileprivate var body: some View {
        VStack {
            ...
            HStack {
                ...
                Button(
                    action: {
                        voicRecorderViewModel.recordBtnTapped()
                    },
                    label: {
                        if voicRecorderViewModel.isRecording {
                            Image("micRecording")
