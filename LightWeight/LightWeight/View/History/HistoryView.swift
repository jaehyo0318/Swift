  ...
  var body: some View {
      ZStack{
          ...
          VStack(alignment: .leading){
              ...
              Text("\(Self.calendarHeaderDateFormatter.string(from: month)) 운동기록")
                  .font(.system(size: 26))
                .foregroundStyle(Color.white)
    ...
    VStack {
      ...
      HStack {
        ForEach(Self.weekdaySymbols.indices, id: \.self) { symbol in
            if Self.weekdaySymbols[symbol] == "Sun" {
                Text(Self.weekdaySymbols[symbol].uppercased())
                .foregroundColor(.red)
                .frame(maxWidth: .infinity)
            } else if Self.weekdaySymbols[symbol] == "Sat" {
                Text(Self.weekdaySymbols[symbol].uppercased())
      ...
      Button(
        ...
        label: {
          Image(systemName: "chevron.left")
            .font(.title)
            .foregroundColor(canMoveToPreviousMonth() ? .white : .gray)
        }
      )
      ...
      Button(
        ...
        label: {
          Image(systemName: "chevron.right")
            .font(.title)
            .foregroundColor(canMoveToNextMonth() ? .white : .gray)
        }
      )
